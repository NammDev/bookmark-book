...
import Loader, { PublicShareIcon } from './icons'
import { updateSharedTag } from '@/lib/actions/shared'
...
export default function SharePopover({ tag, children, className }: SharePopoverProp) {
  const [open, setOpen] = useState(false)
  const [isCopied, setIsCopied] = useState(false)
  const [loading, setLoading] = useState(false)
  const [randomHash, setRandomHash] = useState(tag?.sharedHash || tag?.name)
  ...
  const generateSharableUrl = async () => {
    try {
      setLoading(true)
      if (user?.shareCount >= currentPlan.limit.share && !shared) {
        toast.error(`Sharing limit reached! Upgrade to pro plan.`)
        return
      }
      const updatedTag = await updateSharedTag(tag, !shared)

      setRandomHash(updatedTag?.sharedHash as string)
      setShared(updatedTag?.shared as boolean)
    } catch {
